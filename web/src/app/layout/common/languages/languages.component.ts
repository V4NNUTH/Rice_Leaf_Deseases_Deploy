...
export class LanguagesComponent implements OnInit, OnDestroy {
    ...
    private _updateNavigation(lang: string): void {
        ...
        // Get the pos item and update its title
        const posItam = this._helpersNavigationService.getItem('pos', navigation);
        if (posItam) {
            this._translocoService.selectTranslate('Pos').pipe(take(1)).subscribe((translation) => {
                // Set the title
        ...
        // Get the sales item and update its title
        const saleItem = this._helpersNavigationService.getItem('sale', navigation);
        if (saleItem) {
            this._translocoService.selectTranslate('Sale').pipe(take(1)).subscribe((translation) => {
                // Set the title
        ...
        // Get the product item and update its title
        const productItem = this._helpersNavigationService.getItem('product', navigation);
        if (productItem) {
            this._translocoService.selectTranslate('Product').pipe(take(1)).subscribe((translation) => {
                // Set the title
        ...
        // Get the all products item and update its title
        const allProductItem = this._helpersNavigationService.getItem('all-product', navigation);
        if (allProductItem) {
            this._translocoService.selectTranslate('All Products').pipe(take(1)).subscribe((translation) => {
                // Set the title
        ...
        // Get the product type item and update its title
        const productTypeItem = this._helpersNavigationService.getItem('product-type', navigation);
        if (productTypeItem) {
            this._translocoService.selectTranslate('Product Type').pipe(take(1)).subscribe((translation) => {
                // Set the title
        ...
        // Get the user item and update its title
        const userItem = this._helpersNavigationService.getItem('user', navigation);
        if (userItem) {
            this._translocoService.selectTranslate('Users').pipe(take(1)).subscribe((translation) => {
                // Set the title
        ...
        if (myProfileItem) {
            this._translocoService.selectTranslate('Profile').pipe(take(1)).subscribe((translation) => {
                // Set the title
